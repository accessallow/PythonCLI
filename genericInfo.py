...
def generic_info(app, service_drni_id):
    rels = type_of_rels(app, service_drni_id)
    for r in rels:
        print("Node(s) with Reletionship : {0}".format(r))
        print_connnected_table(app, service_drni_id,r)


def type_of_rels(app, service_drni_id):
    print("\nReletionships with this node:")
    rellist = []
    with app.driver.session() as session:
            result = session.read_transaction(_type_of_rels, service_drni_id)
...
def _type_of_rels(tx, network_drni_id):
    query = (
            "match (n)-[r]-(m) where n.drniId = {0} return distinct TYPE(r) as reltype, count(r) as count"
...
def _rel_items(tx, network_drni_id,rel_name):
    query = (
        "match (n)-[r:{0}]-(m) where n.drniId = {1} "
